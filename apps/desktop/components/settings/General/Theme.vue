...
<template>
	<div class="grid grid-cols-2 gap-4">
		<div class="flex items-center justify-end">
			<span class="justify-self-end">{{ $t("settings.general.theme") }}</span>
		</div>
		<Dialog>
			<DialogTrigger as-child>
				<Button variant="outline" class="w-36">{{ $t("settings.general.editStyle") }}</Button>
			</DialogTrigger>
			<DialogContent class="sm:max-w-[25rem]">
				<DialogHeader>
					<DialogTitle v-show="false">Edit Theme</DialogTitle>
