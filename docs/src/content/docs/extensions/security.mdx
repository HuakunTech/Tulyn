...
Since community maintained extensions can be installed to Jarvis, there is no guarantee that the extension is safe,
...
All extensions are open sourced in GitHub Repo [JarvisExtensions](https://github.com/HuakunTech/JarvisExtensions/) for transparency and security review.
...
Jarvis also takes advantages of system APIs to provide powerful features, but it also has a multi-level
permission management system to control what an extension can do.

Jarvis is designed to take a similar approach to Chrome extensions, where the required permission for an app must be declared in the manifest file.
...
Jarvis splits exposed APIs into different permission groups. e.g. clipboard APIs have `clipboard-read` and `clipboard-write` permissions.
...
Fore more details about permission declaration, see [Extension Permissions](./permissions.dx).
...
All extensions are open sourced in GitHub Repo [JarvisExtensions](https://github.com/HuakunTech/JarvisExtensions/).

It's a pnpm monorepo, CI will build all extensions to make sure they can be built.
